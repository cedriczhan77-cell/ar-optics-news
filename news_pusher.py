...
def push_to_feishu(payload: dict) -> bool:
    """推送到所有已配置的飞书 Webhook"""
    webhooks = [w for w in [FEISHU_WEBHOOK, FEISHU_WEBHOOK_2] if w and "YOUR_HOOK_ID" not in w]
    if not webhooks:
        print("❌ 未配置任何有效的 Webhook")
        return False
    ...
    for i, url in enumerate(webhooks, 1):
        try:
            ...
            if result.get("code") == 0 or result.get("StatusCode") == 0:
                ...
                print(f"❌ Webhook {i} 推送失败: {result}")
                all_ok = False
        except Exception as e:
            print(f"❌ Webhook {i} 推送异常: {e}")
            all_ok = False
    return all_ok
