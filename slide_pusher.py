...
def push_to_feishu_image(img_path: str, pptx_path: str | None,
    ...
    if token:
        ...
        if pptx_path and os.path.exists(pptx_path):
            file_key  = upload_pptx_to_feishu(pptx_path, token, date_str)
    ...
    card_payload = {
        ...
        "card": {
            "header": {
                "title": {"tag": "plain_text", "content": f"AR 光学日报 | {date_str}"},
                "template": "blue",
            },
    ...
    for i, url in enumerate(webhooks, 1):
        ...
        try:
            ...
            if result.get("code") == 0 or result.get("StatusCode") == 0:
                print(f"  ✅ Webhook {i} 图片卡片推送成功")
            else:
                print(f"  ❌ Webhook {i} 图片卡片失败: {result}")
                all_ok = False
        except Exception as e:
            print(f"  ❌ Webhook {i} 图片卡片异常: {e}"); all_ok = False
        ...
        if file_payload:
            try:
                r2     = requests.post(url,
                                       headers={"Content-Type": "application/json"},
                                       data=json.dumps(file_payload, ensure_ascii=False),
                                       timeout=30)
                result2 = r2.json()
                if result2.get("code") == 0 or result2.get("StatusCode") == 0:
                    print(f"  ✅ Webhook {i} PPTX 文件推送成功")
                else:
                    print(f"  ❌ Webhook {i} PPTX 文件失败: {result2}")
            except Exception as e:
                print(f"  ❌ Webhook {i} PPTX 文件异常: {e}")
