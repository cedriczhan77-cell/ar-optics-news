...
def push_to_feishu_image(img_path: str, date_str: str, n_paper: int, n_news: int) -> bool:
    ...
    # ── Step 1 & 2：获取 Token → 上传图片 → 得到 image_key ──
    image_key = None
    ...
    if image_key:
        ...
        elements = [
            ...
            },
            {
                ...
                "mode": "fit_horizontal",   # 横向铺满卡片宽度
                "preview": True,            # 支持点击放大
            },
        ]
        print("  📸 使用图片直发模式")
    else:
        # 降级：纯文字卡片（App 未配置时的备用方案）
        elements = [{
            "tag": "markdown",
            "content": (
                f"**📚 文献 {n_paper} 篇  ·  📰 新闻 {n_news} 篇**\n"
                f"#光学  #AR  #Waveguide  #TFLN  #MicroLED\n\n"
                f"⚠️ 图片发送需配置 FEISHU_APP_ID 和 FEISHU_APP_SECRET"
            ),
        }]
        print("  ⚠️  降级为纯文字卡片（未配置 App 凭证）")
